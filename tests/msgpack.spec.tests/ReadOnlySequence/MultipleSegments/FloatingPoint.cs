...
namespace ProGaudi.MsgPack.Tests.ReadOnlySequence.MultipleSegments
{
    public sealed class FloatingPoint
    {
        ...
        {
            MsgPackSpec.TryReadDouble(data.ToMultipleSegments(), out var value, out var readSize).ShouldBeTrue();
        ...
        {
            MsgPackSpec.TryReadFloat(data.ToMultipleSegments(), out var value, out var readSize).ShouldBeTrue();
            value.ShouldBe(number);
            readSize.ShouldBe(data.Length);
        }
    }
