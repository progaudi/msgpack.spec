...
namespace ProGaudi.MsgPack.Tests.ReadOnlySequence.MultipleSegments
{
    public sealed class CopyToken
    {
        ...
        public void NonEmptyRead(byte[] data, int objectsCount)
        {
            ...
            var buffer = data.ToMultipleSegments();
        ...
        {
            var e = Should.Throw<ArgumentOutOfRangeException>(() => MsgPackSpec.ReadToken(System.Array.Empty<byte>().ToMultipleSegments()));
        ...
        {
            var e = Should.Throw<ArgumentOutOfRangeException>(() => MsgPackSpec.ReadToken(new[] {DataCodes.NeverUsed}.ToMultipleSegments()));
            e.Message.ShouldBe(
                $"Data code at sequence[0] is 0xc1 and it is invalid data code.{Environment.NewLine}Parameter name: sequence");
            e.ParamName.ShouldBe("sequence");
        }
    }
}
