...
namespace ProGaudi.MsgPack.Tests.ReadOnlySequence
{
    public sealed class FloatingPoint
    {
        ...
        public void SingleSegmentTestDouble(double number, byte[] data)
        {
        ...
        public void SingleSegmentTestFloat(float number, byte[] data)
        {
        ...
        [InlineData(double.PositiveInfinity, new byte[] {203, 127, 240, 0, 0, 0, 0, 0, 0})]
        [InlineData(double.NegativeInfinity, new byte[] {203, 255, 240, 0, 0, 0, 0, 0, 0})]
        public void MultipleSegmentsTestDouble(double number, byte[] data)
        {
            MsgPackSpec.ReadDouble(data.ToMultipleSegments(), out var readSize).ShouldBe(number);
            readSize.ShouldBe(data.Length);
        }
        ...
        [InlineData(float.PositiveInfinity, new byte[] {202, 127, 128, 0, 0})]
        [InlineData(float.NegativeInfinity, new byte[] {202, 255, 128, 0, 0})]
        public void MultipleSegmentsTestFloat(float number, byte[] data)
        {
            MsgPackSpec.ReadFloat(data.ToMultipleSegments(), out var readSize).ShouldBe(number);
            readSize.ShouldBe(data.Length);
        }
    }
}
