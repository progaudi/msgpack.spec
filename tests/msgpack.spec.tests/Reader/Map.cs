...
namespace ProGaudi.MsgPack.Tests.Reader
{
    public sealed class Map
    {
        ...
        [Fact]
        public void ReadSimpleDictionary()
        {
        ...
        [Fact]
        public void ReadNonFixedDictionary()
        {
            var bytes = new byte[]
            {
                0xde,
        ...
        private static void TestReadDictionary(byte[] bytes, Dictionary<int, string> test)
        {
            ...
            ReadDictionary(test, length, span, readSize);
        }

        [Fact]
        public void TryReadSimpleDictionary()
        {
            var test = new Dictionary<int, string>
            {
                {1, "a"},
            ...
            TestTryReadDictionary(bytes, test);
        }

        [Fact]
        public void TryReadNonFixedDictionary()
        {
            var bytes = new byte[]
            {
                0xde,
        ...
        {
            var span = new ReadOnlySpan<byte>(bytes);
            TryReadMapHeader(span, out var length, out var readSize).ShouldBeTrue();
            ReadDictionary(test, length, span, readSize);
        }

        private static void ReadDictionary(Dictionary<int, string> test, int length, ReadOnlySpan<byte> span, int readSize)
        {
