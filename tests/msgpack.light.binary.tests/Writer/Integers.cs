...
namespace ProGaudi.MsgPack.Light.Tests.Writer
{
    public class Integers
    {
        ...
        {
            var buffer = new Span<byte>(ArrayPool<byte>.Shared.Rent(10));
            var length = MsgPackBinary.WriteInt32(buffer, number);
            length.ShouldBe(data.Length);
            buffer.Slice(0, length).ToArray().ShouldBe(data);
        }
        ...
        {
            var buffer = new Span<byte>(ArrayPool<byte>.Shared.Rent(10));
            var length = MsgPackBinary.WriteInt16(buffer, number);
            length.ShouldBe(data.Length);
            buffer.Slice(0, length).ToArray().ShouldBe(data);
        }
        ...
        {
            var buffer = new Span<byte>(ArrayPool<byte>.Shared.Rent(10));
            var length = MsgPackBinary.WriteInt8(buffer, number);
            length.ShouldBe(data.Length);
            buffer.Slice(0, length).ToArray().ShouldBe(data);
        }
        ...
        {
            var buffer = new Span<byte>(ArrayPool<byte>.Shared.Rent(10));
            var length = MsgPackBinary.WriteUInt32(buffer, number);
            length.ShouldBe(data.Length);
            buffer.Slice(0, length).ToArray().ShouldBe(data);
        }
        ...
        {
            var buffer = new Span<byte>(ArrayPool<byte>.Shared.Rent(10));
            var length = MsgPackBinary.WriteUInt8(buffer, number);
            length.ShouldBe(data.Length);
            buffer.Slice(0, length).ToArray().ShouldBe(data);
        }
    }
