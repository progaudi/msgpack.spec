...
namespace ProGaudi.MsgPack
{
    ...
    public static partial class MsgPackSpec
    {
        ...
        {
            readSize = 9;
            if (buffer[0] != DataCodes.Float64) throw WrongCodeException(buffer[0], DataCodes.Float64);
        ...
        public static double ReadDouble(ReadOnlySpan<byte> buffer, out int readSize)
        {
            ...
            switch (code)
            {
                ...
                default:
                    throw WrongCodeException(code, DataCodes.Float64, DataCodes.Float32);
            }
        }
        ...
        /// <param name="value">Value read from <paramref name="buffer"/>. If return value is false, value is unspecified.</param>
        /// <param name="readSize">Count of bytes, read from <paramref name="buffer"/>. If return value is false, value is unspecified.</param>
        /// <returns><c>true</c>, if everything is ok. <c>false</c> if buffer is too small, or <paramref name="buffer[0]"/> is not <see cref="DataCodes.Float32"/> or <see cref="DataCodes.Float64"/></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
